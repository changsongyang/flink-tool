...
public class FlinkRedisSink<IN> extends RichSinkFunction<HashMap<String, String>> {
    ...
    public FlinkRedisSink(DeserializationSchema<IN> deserializationSchema, Jedis jedis, RedisConnectionConfig redisConnectionConfig) {
        this.deserializationSchema = deserializationSchema;
        this.jedis = jedis;
        this.redisConnectionConfig = redisConnectionConfig;
    }

    public FlinkRedisSink(DeserializationSchema<IN> deserializationSchema, RedisConnectionConfig redisConnectionConfig, Jedis jedis, int expireSecond) {
        this.deserializationSchema = deserializationSchema;
        this.redisConnectionConfig = redisConnectionConfig;
        this.jedis = jedis;
    ...
    public void invoke(HashMap<String, String> map, Context context) throws Exception {
        log.info("【insertData】{}", map.toString());
        for (Map.Entry<String, String> entry : map.entrySet()) {
            String insertResult = "";
            // 设置过期时间
            if (this.expireSecond != 0) {
                // NX是不存在时才set， XX是存在时才set， EX是秒，PX是毫秒
                insertResult = jedis.set(
                        entry.getKey(),
                        entry.getValue(),
                        "NX",
                        "EX",
